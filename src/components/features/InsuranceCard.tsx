...
const InsuranceCard: React.FC<InsuranceCardProps> = ({ insurance }) => {
  const formattedDate = new Date(insurance.createdAt).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric'
  });
    ...
    <Card className="hover:translate-y-[-4px] transition-transform duration-300">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-gray-50 p-3 rounded-lg">
          ...
          <div className="font-bold text-lg">${insurance.monthlyPremium.toFixed(2)}</div>
        </div>
        ...
        <div className="bg-gray-50 p-3 rounded-lg">
          ...
          <div className="font-bold text-lg">${insurance.yearlyPremium.toFixed(2)}</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex justify-between">
          ...
          <span className="font-medium">${insurance.coverageAmount.toLocaleString()}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium">${insurance.deductible.toLocaleString()}</span>
        </div>
